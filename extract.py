...
def features(df, num_features):
    ...
    '''
    df = df.copy()
    # df['FLUX'] = measurements.__mag_to_flux__(df.FLUX)
    ...
    feature_dict['small_kurtosis'] = measurements.small_kurtosis(df.FLUX)
    feature_dict['std'] = measurements.std(df.FLUX)
    feature_dict['beyond1st'] = measurements.beyond1st(df.FLUX, df.FLUX_ERROR)
    feature_dict['stetson_j'] = measurements.stetson_j(
        df.FLUX, df.FLUX_ERROR, df.Date)
    feature_dict['stetson_k'] = measurements.stetson_k(df.FLUX, df.FLUX_ERROR)
    feature_dict['max_slope'] = measurements.max_slope(df.FLUX, df.Date)
    feature_dict['amplitude'] = measurements.amplitude(df.FLUX)
